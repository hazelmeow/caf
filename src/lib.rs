    ...
    T: Read + Seek,
{
    ...
    */
    pub fn read_chunks_to_mem(
        &mut self,
        mut required: Vec<ChunkType>,
    ...
    T: Read + Seek,
{
    ...
    pub fn from_chunk_reader(
        mut ch_rdr: CafChunkReader<T>,
        mut filter_by: Vec<ChunkType>,
        ...
        let p_table_required =
            audio_desc.bytes_per_packet == 0 || audio_desc.frames_per_packet == 0;
        let packet_table = match packet_table_idx {
            Some(i) => Some(remove_and_unwrap!(i, PacketTable)),
            None if p_table_required => {
                let (chunks, hdrs) =
                    try!(ch_rdr.read_chunks_to_mem(vec![ChunkType::PacketTable], &content_read));
    ...
    pub fn next_packet_size(&self) -> Option<usize> {
        ...
        if self.audio_chunk_len != -1 && self.audio_chunk_offs + res as i64 > self.audio_chunk_len {
    ...
    pub fn seek_to_packet(&mut self, packet_idx: usize) -> Result<(), CafError> {
        ...
        let offs: i64 = match self.audio_desc.bytes_per_packet {
            0 => self.packet_table.as_ref().unwrap().lengths[min_idx..max_idx]
                .iter()
                .map(|v| *v as i64)
                .sum(),
